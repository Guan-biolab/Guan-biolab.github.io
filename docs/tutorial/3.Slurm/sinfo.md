...
• ­S 、­­sort=：设定显示信息的排序方式。排序字段参见后面 输出格式部分，多个排序字段采用,分隔，字段前面的+和­分表表示升序（默认） 或降序。队列字段P前面如有#，表示以Slurm配置文件slurm.conf中的顺序显示。 例如：sinfo ­S +P,­m表示以队列名升序及内存大小降序排序。
...
­-O , ­­Format=：按照格式输出信 息，类似­o 、­­format=。 每个字段的格式为“type[:[.]size]”
